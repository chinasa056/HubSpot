...
exports.registerHostValidator = (req, res, next) => {
  const schema = joi.object({
    ...
    companyAddress: joi.string().min(6).trim().required().messages({
      "any.required": "companyAddress is required.",
      "string.empty": "companyAddress cannot be empty.",
    }),
    meansOfIdentification: joi.string().trim().required().messages({
      "any.required": "meansOfIdentification is required.",
      "string.empty": "meansOfIdentification cannot be empty.",
    }),
    idCardNumber: joi.number().integer().min(10000000000).required().messages({
      "any.required": "idCardNumber is required.",
      "number.base": "idCardNumber must be a valid number.",
      "number.integer": "idCardNumber must be an integer.",
      "number.min": "idCardNumber must be at least 11 digits long.",
    }),
  });
